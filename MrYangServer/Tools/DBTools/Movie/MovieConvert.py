...
movie_item_config = XMLMedia.item_info_dict()
...
def insert_gallery_info():
    ...
    for movie_info in movie_infos:
        if movie_info.rel_path and movie_info.rel_path in movie_item_config:
            ...
            value = tuple(eval(m_cfg[XMLMedia.ITEM_TAGS.PIXEL]))
            data.source_size = int(value[1])
            data.fps = m_cfg[XMLMedia.ITEM_TAGS.FPS]
...
def create_db(path, info):
    ...
    is_dir = True
    if movie_config[XMLMedia.TAGS.DIR_EXTEN] in name:
        is_dir = False
...
if __name__ == '__main__':
    ...
    dict = ypath.path_result(desc_root, movie_config[XMLMedia.TAGS.TS_DIR], parse_file=False)
