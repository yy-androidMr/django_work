...
def create_media_item_info_xml(item_info_list):
    if len(item_info_list) == 0:
        return
    media_item_info_path = get_infos()[TAGS.ITEM_INFO]
    doc = minidom.Document()
    booklist = doc.createElement(CONFIG_NAME)
    doc.appendChild(booklist)
    booklist.appendChild(doc.createTextNode('\n'))
    path, _ = XMLBase.get_cfg_dir()
    media_item_info_path = path + media_item_info_path
    ypath.create_dirs(media_item_info_path)
    ...
    with open(media_item_info_path, 'w', encoding='UTF-8') as fh:
...
def item_info_dict():
    ...
    nodes = XMLBase.parse(media_item_info_path).elem.xml_nodes(ITEM_TAG)
    for node in nodes:
        items = {}
